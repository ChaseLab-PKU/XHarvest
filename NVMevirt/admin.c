...
static void __nvmev_admin_create_sq(int eid)
{
	...
	struct nvmev_request_queue *req_que;
	unsigned int num_pages, i;
	int dbs_idx;

	sq = kzalloc(sizeof(struct nvmev_submission_queue), GFP_KERNEL);
	req_que = kzalloc(sizeof(struct nvmev_request_queue), GFP_KERNEL);
	...
	sq->tag_mapper = kzalloc(sizeof(__u16) * 65537, GFP_KERNEL);
	ASSERT(sq->tag_mapper);

	INIT_LIST_HEAD(&sq->free_req_list);
	ASSERT(list_empty(&sq->free_req_list));

	req_que->qid = sq->qid;
	req_que->cqid = sq->cqid;
	// req_que->tail = 0;
	req_que->internal_req = kzalloc(sizeof(struct nvmev_internal_request) * sq->queue_size + 1, GFP_KERNEL);

	ASSERT(sq->sq && req_que->internal_req);

	for (i = 0; i <= sq->queue_size; i++) {
		req_que->internal_req[i].interl_req_id = i;
		if (i != 0)
			list_add_tail(&req_que->internal_req[i].list, &sq->free_req_list);
	}
	...
	nvmev_vdev->reqqs[sq->qid] = req_que;
...
static void __nvmev_admin_delete_sq(int eid)
{
	...
	req_que = nvmev_vdev->reqqs[qid];
	nvmev_vdev->reqqs[qid] = NULL;

	if (req_que) {
		kfree(req_que->internal_req);
		kfree(req_que);
	}
