 ...
 */
#define CONFIG_NVMEVIRT_IDLE_TIMEOUT 120
...
#define CONFIG_NVMEV_ERROR_DEBUG
#ifdef CONFIG_NVMEV_ERROR_DEBUG
...
#define CONFIG_NVMEV_EXIT_DEBUG
#ifdef CONFIG_NVMEV_EXIT_DEBUG
#define NVMEV_EXIT_LOG(string, args...) printk(KERN_INFO "%s: " string, NVMEV_DRV_NAME, ##args)
#else
#define NVMEV_EXIT_LOG(string, args...)
#endif

#define CONFIG_NVMEV_PARAM_LOG
#ifdef CONFIG_NVMEV_PARAM_LOG
#define NVMEV_PARAM_LOG(string, args...) printk(KERN_ERR "[PARAM LOG] %s: " string, NVMEV_DRV_NAME, ##args)
#else
#define NVMEV_PARAM_LOG(string, args...)
#endif

#define CONFIG_NVMEV_DISPATCHER_LOG
#ifdef CONFIG_NVMEV_DISPATCHER_LOG
#define NVMEV_DISPATCHER_LOG(string, args...) printk(KERN_ERR "[Dispt LOG] %s: " string, NVMEV_DRV_NAME, ##args)
#else
#define NVMEV_DISPATCHER_LOG(string, args...)
#endif

// #define CONFIG_NVMEV_HACK_LOG
#ifdef CONFIG_NVMEV_HACK_LOG
#define NVMEV_HACK_LOG(string, args...) printk(KERN_ERR "[HACK LOG] %s: " string, NVMEV_DRV_NAME, ##args)
#else
#define NVMEV_HACK_LOG(string, args...)
#endif

#define LOG_PRINT_INTERVAL 32768

// #define CONFIG_NVMEV_INTR_LOG
#ifdef CONFIG_NVMEV_INTR_LOG
#define NVMEV_INTR_LOG(string, args...) printk(KERN_ERR "[INTR LOG] %s: " string, NVMEV_DRV_NAME, ##args)
#else
#define NVMEV_INTR_LOG(string, args...)
#endif

// #define CONFIG_NVMEV_QUEUE_LOG
#ifdef CONFIG_NVMEV_QUEUE_LOG
#define NVMEV_QUEUE_LOG(string, args...) printk(KERN_ERR "[CQ LOG] %s: " string, NVMEV_DRV_NAME, ##args)
#else
#define NVMEV_QUEUE_LOG(string, args...)
#endif

// #define CONFIG_NVMEV_FINE_LOG
#ifdef CONFIG_NVMEV_FINE_LOG
#define NVMEV_FINE_LOG(string, args...) printk(KERN_ERR "[REQ LOG] %s: " string, NVMEV_DRV_NAME, ##args)
#else
#define NVMEV_FINE_LOG(string, args...)
#endif

#define NVMEV_WARNING(string, args...) printk(KERN_WARNING "%s: " string, NVMEV_DRV_NAME, ##args)
...
struct nvmev_io_cmd {
	...
	uint64_t slpa;
	int nlb;
	atomic_t is_completed;
	uint64_t nsecs;
	uint64_t revserv;
};

_Static_assert(sizeof(struct nvmev_io_cmd) == 64, "Size of nvmev_io_cmd is not 64 bytes");

struct nvmev_internal_request {
	bool is_flight;
	int command_id;
	int remaining_req;
	int interl_req_id;
	...
	uint64_t nsecs_end;
	struct nvmev_io_cmd io_cmd;
};

struct nvmev_request_queue {
	int qid;
	int cqid;

	struct nvmev_internal_request *internal_req;
};
...
struct nvmev_config {
	...
	unsigned int cpu_nr_dispatcher[32];
	unsigned int nr_io_workers;
	unsigned int nr_SGX_io_workers;
	unsigned int nr_dispatcher;
	...
	unsigned int FTL_split_units;
	unsigned int split_ratio;
	unsigned int seg_units[21];

	unsigned int in_que_size, out_que_size;
...
struct nvmev_dev {
	...
	struct nvmev_dispatcher_ctx *dispatcher_ctxes;
	...
	struct nvmev_io_cmd ***in_que, ***out_que;
	int **in_que_tail, **out_que_head;
	...
	struct nvmev_request_queue *reqqs[NR_MAX_IO_QUEUE + 1];
	...
	struct proc_dir_entry *proc_worker_queue;
	struct proc_dir_entry *proc_ssd_size;
	struct proc_dir_entry *proc_thread_config;
...
int nvmev_proc_io_sq(int dispatcher_id, int qid, int new_db, int old_db);
